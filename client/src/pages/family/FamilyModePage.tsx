...
const FamilyModePage: React.FC = () => {
  ...
  const [showMemberModal, setShowMemberModal] = useState<FamilyMember | null>(null);
  const [showBudgetModal, setShowBudgetModal] = useState(false);
  const [activeTab, setActiveTab] = useState<'overview' | 'members' | 'expenses' | 'budget' | 'reminders'>('overview');
  const [selectedMember, setSelectedMember] = useState<string | null>(null);
  const [isRefreshing, setIsRefreshing] = useState(false);
  const [copiedCode, setCopiedCode] = useState(false);

  const [createForm, setCreateForm] = useState({ familyName: '', nickname: user?.fullName || '', relation: 'Father' as const, budgetAmount: 50000 });
  const [joinForm, setJoinForm] = useState({ familyCode: '', nickname: user?.fullName || '', relation: 'Other' as const });
  const [inviteForm, setInviteForm] = useState({ email: '', relation: 'Other' as const, role: 'Member' as const });

  const loadFamilyData = useCallback(async () => {
    try {
      setIsLoading(true);
      const response = await familyApi.getDashboard();
      if (response.success && response.data) { setDashboardData(response.data); setHasFamily(true); }
      else { setHasFamily(false); setDashboardData(null); }
    } catch (error: any) {
      if (error.response?.status === 404) { setHasFamily(false); setDashboardData(null); }
      else { toast.error('Failed to load family data'); }
    } finally { setIsLoading(false); }
  }, []);

  useEffect(() => { loadFamilyData(); }, [loadFamilyData]);

  const handleRefresh = async () => { setIsRefreshing(true); await loadFamilyData(); setIsRefreshing(false); toast.success('Data synced'); };

  const handleCopyCode = () => {
    if (dashboardData?.family.familyCode) {
      navigator.clipboard.writeText(dashboardData.family.familyCode);
      setCopiedCode(true); toast.success('Family code copied!');
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-2">
              <button onClick={handleCopyCode} className="hidden sm:flex items-center gap-2 px-3 py-2 bg-white/10 rounded-xl border border-white/10 hover:bg-white/20 transition-all">
                <span className="text-white/60 text-xs">Code:</span><span className="font-mono font-bold text-white tracking-wider">{dashboardData?.family.familyCode}</span>
        ...
        <AnimatePresence mode="wait">
                ...
                <div className="bg-white/5 backdrop-blur-sm border border-white/10 rounded-2xl p-6">
                  ...
                  {dashboardData?.family.members.filter(m => m.role === 'Admin').slice(0, 1).map((admin) => (
                    <div key={admin.userId} className="flex items-center gap-4">
                      <div className="w-16 h-16 rounded-2xl bg-gradient-to-br from-amber-400/20 to-orange-500/20 flex items-center justify-center text-4xl border-2 border-amber-500/30">{admin.avatar || relationAvatars[admin.relation]}</div>
          ...
          {activeTab === 'members' && (
            <motion.div key="members" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className="space-y-6">
              ...
              <div className="bg-white/5 backdrop-blur-sm border border-white/10 rounded-2xl p-6">
                <div className="flex items-center justify-between mb-4"><div className="flex items-center gap-3"><div className="p-2 bg-purple-500/20 rounded-lg"><Share2 className="w-5 h-5 text-purple-400" /></div><div><h3 className="text-white font-semibold">Invite Code</h3><p className="text-white/60 text-sm">Share this code with family members to join</p></div></div></div>
                <div className="flex items-center gap-4">
                  <div className="flex-1 bg-white/10 rounded-xl p-4 flex items-center justify-center"><span className="font-mono text-3xl font-bold text-white tracking-widest">{dashboardData?.family.familyCode}</span></div>
                  <div className="flex flex-col gap-2">
                    <button onClick={handleCopyCode} className="flex items-center gap-2 px-4 py-2 bg-white/10 rounded-xl hover:bg-white/20 transition-all">{copiedCode ? <CheckCircle className="w-4 h-4 text-green-400" /> : <Copy className="w-4 h-4 text-white" />}<span className="text-white text-sm">{copiedCode ? 'Copied!' : 'Copy'}</span></button>
...
const MemberCard: React.FC<{ member: FamilyMember; isCurrentUser: boolean; isAdmin: boolean; formatCurrency: (n: number) => string; formatDate: (d: string) => string; onEdit: () => void; onViewExpenses: () => void; }> = ({ member, isCurrentUser, isAdmin, formatCurrency, formatDate, onEdit, onViewExpenses }) => (
  <div className={`bg-white/5 backdrop-blur-sm border rounded-2xl p-5 transition-all hover:bg-white/10 ${isCurrentUser ? 'border-blue-500/30 ring-1 ring-blue-500/20' : 'border-white/10'}`}>
    <div className="flex items-start justify-between mb-4">
      <div className="flex items-center gap-3">
        <div className="w-14 h-14 rounded-2xl bg-gradient-to-br from-white/10 to-white/5 flex items-center justify-center text-3xl border border-white/10">{member.avatar || relationAvatars[member.relation]}</div>
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
      <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="bg-slate-900 border border-white/10 rounded-3xl w-full max-w-md overflow-hidden">
        ...
        <div className="p-6 space-y-4">
          <div className="bg-gradient-to-br from-purple-500/20 to-blue-500/20 border border-purple-500/20 rounded-xl p-4"><p className="text-white/80 text-sm mb-2">Share this code with them:</p><div className="flex items-center justify-between bg-white/10 rounded-lg p-3"><span className="font-mono text-xl font-bold text-white tracking-wider">{familyCode}</span><button onClick={() => { navigator.clipboard.writeText(familyCode || ''); toast.success('Copied!'); }} className="p-2 hover:bg-white/10 rounded-lg transition-all"><Copy className="w-4 h-4 text-white" /></button></div></div>
